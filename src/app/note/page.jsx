    ...
    <nav className="grid gap-1 px-2">
      {
        bookList.map((book, index) => (
          <Button variant={ book.id === currentBook? '': 'ghost' } className="flex h-8" key={book.id} onClick={onBookClick(book.id)}>
    ...
    <nav className="grid gap-1 px-2">
      {
        noteList.map((note, index) => (
          <Button variant={ note.id === currentNote?.id ? '': 'ghost' } className="flex h-8 justify-start" key={index} onClick={() => setCurrentNote(note)}>
  ...
  return (
    <div className="flex flex-1 border border-black m-4 rounded-lg overflow-hidden">
      ...
      <div className="flex-1">
        <div className="border-b border-solid border-black h-12 flex items-center pl-4">
          { currentNote?.name }
          {
            readOnly
            ?
